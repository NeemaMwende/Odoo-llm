...
class LLMGenerationJob(models.Model):
    ...
    external_job_id = fields.Char(
        ...
        help="Provider's job identifier"
    )
    provider_data = fields.Json(
        string="Provider Data",
        help="Provider-specific metadata and configuration"
    )
    
    # Generation inputs
    generation_inputs = fields.Json(
        string="Generation Inputs",
        help="Input data for the generation process"
    )
    
    # Timing fields
    queued_at = fields.Datetime(
        string="Queued At",
        help="When the job was queued"
    )
    started_at = fields.Datetime(
        string="Started At",
        help="When the job started processing"
    )
    completed_at = fields.Datetime(
        string="Completed At",
        help="When the job completed (success or failure)"
    )
    ...
    def write(self, vals):
        ...
        if 'state' in vals:
            ...
            elif vals['state'] in ['completed', 'failed', 'cancelled'] and not vals.get('completed_at'):
    ...
    def get_prepared_inputs(self):
        ...
        if hasattr(self.thread_id, 'prepare_generation_inputs'):
            ...
            attachment_ids = None
            if self.input_message_id and hasattr(self.input_message_id, 'attachment_ids'):
