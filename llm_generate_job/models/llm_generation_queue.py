...
class LLMGenerationQueue(models.Model):
    ...
    def action_clear_queue(self):
        ...
        if queued_jobs:
            ...
            _logger.info(f"Cleared {len(queued_jobs)} jobs from queue for model {self.model_id.name}")
    ...
    def action_retry_failed_jobs(self):
        ...
        _logger.info(f"Retried {retried_count} failed jobs for model {self.model_id.name}")
    ...
    def _process_all_queues(self):
        ...
        for queue in self.search([('enabled', '=', True)]):
            try:
                ...
                _logger.debug(f"Processed {processed} jobs for model {queue.model_id.name}")
            except Exception as e:
                _logger.error(f"Error processing queue for model {queue.model_id.name}: {e}")
    ...
    def action_view_today_jobs(self):
        ...
        return {
            ...
            'res_model': 'llm.generation.job',
            'view_mode': 'tree,form',
            'domain': [('model_id', '=', self.model_id.id), ('create_date', '>=', today)],
            'context': {'default_model_id': self.model_id.id},
        }
