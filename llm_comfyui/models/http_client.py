...
class ComfyUIClient:
                ...
                try:
                    if hasattr(e.response, 'content') and e.response.content:
                        try:
                            ...
                            _logger.error(f"Response content: {json.dumps(content, indent=2)}")
                        except:
                            # If not JSON, log as text
                            content = e.response.content.decode('utf-8', errors='replace')
                            _logger.error(f"Response content: {content}")
                except Exception as content_error:
                    _logger.error(f"Could not extract response content: {content_error}")
    ...
    def upload_image(self, image_data, filename=None, subfolder=None, type="input", overwrite=False):
    ...
    def poll_prompt_status(self, prompt_id, max_attempts=30, delay=5):
        ...
        for attempt in range(max_attempts):
            ...
            is_running = any(item[1] == prompt_id for item in queue.get("queue_running", []))
            is_pending = any(item[1] == prompt_id for item in queue.get("queue_pending", []))
                ...
                if prompt_id in history:
                    ...
                    if "outputs" in prompt_data and prompt_data["outputs"]:
                        ...
                        if isinstance(outputs, dict) and any("images" in node_output for node_output in outputs.values()):
                            _logger.info(f"Prompt {prompt_id} completed with valid image outputs")
                            return prompt_data
                        else:
                            _logger.info(f"Prompt {prompt_id} completed but no image outputs found yet, waiting...")
                    else:
                        ...
                        _logger.info(f"Prompt {prompt_id} found in history but no outputs yet, waiting...")
            
            _logger.info(f"Attempt {attempt + 1}/{max_attempts}: Prompt {prompt_id} still processing")
            time.sleep(delay)
            
        raise TimeoutError(f"Timeout waiting for ComfyUI prompt {prompt_id} to complete after {max_attempts} attempts")
