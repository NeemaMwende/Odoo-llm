...
class MailMessage(models.Model):
    ...
    def set_user_vote(self, vote_value):
        ...
        if self.llm_role not in ('assistant', 'tool'):
            raise UserError(_("Voting is only allowed on messages generated by the assistant or a tool."))
        if not isinstance(vote_value, int) or vote_value not in [-1, 0, 1]:
            raise ValidationError(
                _("Invalid vote value. Must be an integer: 1 (up), -1 (down), or 0 (none).")
            )
